...
const create = (req, res, next) => {
  ...
  const { first_name, last_name, email, employment_status, employer, pronouns } = req.body;
  return database('users').insert({ first_name, last_name, email, employment_status, employer, pronouns }).returning('*').then((users) => {
    const [ json ] = users;
    ...
    if (identifying_info.length) {
      ...
      return database('identifying_info').select().whereIn('name', identifying_info.map((i) => i.name )).then((existing_info) => {
        ...
        return database('identifying_info').insert(new_info).returning('id');
      }).then((info_ids) => {
        return database('user_identifying_info').insert(info_ids.map((id) => ({ identifying_info_id: id, user_id: users[0].id })))
      }).then(() => { return res.json(json) });
    }
